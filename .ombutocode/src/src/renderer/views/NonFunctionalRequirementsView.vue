  ...
  <div class="view-container">
    <header class="title-bar">
      <div class="title-copy">
        <p class="folder-path">docs / Non-Functional Requirements /</p>
        <h1 class="file-name">Non-Functional Requirements</h1>
      </div>
      <div class="title-actions">
        <button class="tool-btn" @click="addRow" title="Add Non-Functional Requirement">
          <span class="mdi mdi-plus"></span> Add NFR
        </button>
      ...
      <table class="fr-table">
        ...
        <tbody>
          <tr v-if="rows.length === 0">
            <td colspan="5" class="empty-row">No non-functional requirements yet. Click "Add NFR" to create one.</td>
          </tr>
          <tr
            v-for="(row, i) in rows"
            :key="row.id"
          ...
          >
            ...
            <td class="col-actions">
              <template v-if="editingIdx === i">
                <button class="row-btn" title="Save" @click="commitEdit">
                  <span class="mdi mdi-check"></span>
                </button>
                <button class="row-btn" title="Cancel" @click="cancelEdit">
                  <span class="mdi mdi-close"></span>
                </button>
              </template>
              <template v-else>
                <button class="row-btn" title="Edit" @click="startEdit(i)">
                  <span class="mdi mdi-pencil"></span>
                </button>
                <button class="row-btn delete-btn" title="Delete" @click="deleteRow(i)">
...
const FILE_PATH = 'Non-Functional Requirements/NonFunctionalRequirements.md';
...
function serializeTable(rows) {
  ...
  for (const r of rows) {
    md += `| ${r.id} | ${r.subsystem} | ${r.description} | ${r.status} |\n`;
  }
...
function nextId(rows) {
  let max = 0;
  for (const r of rows) {
    const m = r.id.match(/^NFR-(\d+)$/);
    if (m) {
      const n = parseInt(m[1]);
      if (n > max) max = n;
    }
  }
  return 'NFR-' + String(max + 1).padStart(3, '0');
}
...
export default {
  name: 'NonFunctionalRequirementsView',
  setup() {
    ...
    async function loadFile() {
      ...
      try {
        ...
        rows.value = parseTable(content);
      } catch (e) {
        rows.value = [];
      }

      try {
        const structContent = await window.electron.ipcRenderer.invoke('filetree:readFile', STRUCTURE_PATH);
        subsystems.value = parseSubsystems(structContent);
      } catch (e) {
        subsystems.value = [];
      }
    ...
    function addRow() {
      const id = nextId(rows.value);
      rows.value.push({ id, subsystem: '', description: '', status: 'Draft' });
      dirty.value = true;
      saved.value = false;
      startEdit(rows.value.length - 1);
    }

    function startEdit(i) {
      editingIdx.value = i;
      editRow.value = { ...rows.value[i] };
    }

    function commitEdit() {
      if (editingIdx.value === null) return;
      const i = editingIdx.value;
      rows.value[i] = { ...editRow.value };
    ...
    async function onSave() {
      ...
      saving.value = true;
      try {
        const content = serializeTable(rows.value);
    ...
    async function exportToExcel() {
      ...
      try {
        ...
        await window.electron.ipcRenderer.invoke('excel:exportRequirements', { rows: plainRows, title: 'Non-Functional Requirements' });
    ...
    function onKeyDown(e) {
      ...
      if (e.key === 'Enter' && editingIdx.value !== null) {
        commitEdit();
      }
    ...
    return {
      ...
      rows, subsystems, STATUSES,
      editingIdx, editRow,
      addRow, startEdit, commitEdit, cancelEdit, deleteRow,
      onSave, exportToExcel, importFromExcel,
    };
  },
};
</script>
