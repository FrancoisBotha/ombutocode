...
The repository's code map lives in `docs/Code Map/` — `codemap.json` (machine-readable nodes, edges, and flows), `codemap.html` (the interactive view), and `codemap.lock` (per-module fingerprints). It is generated by the **Code Map** skill.

Use it while decomposing. `codemap.json` tells you which modules exist, what calls what, and which tests cover each module — so epic boundaries follow the real structure of the code rather than a guess at it. If `docs/Code Map/` is missing or its `codemap.lock` no longer matches the repo, say so in your proposal instead of relying on a stale map.

The code map is an **epic-level** artifact. Feature tickets read it; they never rebuild it. Regeneration happens once, in the epic's final closeout ticket, so the cost is paid per epic rather than per ticket.

Every epic you write MUST carry this rule in its **§13 Implementation Notes**, verbatim:
...
This is what stops a ticket from editing a module without knowing its callers, its blast radius, or the tests that guard it — without making every ticket pay to rebuild the map.
