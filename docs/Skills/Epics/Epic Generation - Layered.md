...
## Decomposition strategy — layered

This is the **layered** variant. Epics are organised around subsystems and architectural layers: the foundation first, then the pieces that stand on it.

Typical shape:
1. App shell / scaffold
2. Data model and persistence
3. Authentication
4. Domain services / API surface
5. Feature screens that consume all of the above

**What this optimises for: parallel execution.** Once the foundational epics are `DONE`, many later epics touch different subsystems and different files, so the scheduler can run several agents concurrently with few merge conflicts. Interfaces are designed once, up front, rather than discovered slice by slice; shared components get built once instead of growing organically.

**What you give up.** The product is not usable until the layers meet. An epic can be complete and verified in isolation while delivering nothing a user can see — the value of epic 1 may only materialise in epic 5. Integration risk is concentrated late, and a wrong assumption made in the foundation is expensive by the time a feature exposes it.

**Choose this when** the requirements are well understood and stable, the architecture is settled, throughput matters more than early feedback, or you are building a platform/library/back-end where "what the user can do" is not the unit of progress.

**Choose the vertical-slice variant instead when** you want a working, demoable product at the end of every epic, the requirements will evolve as you see the thing running, or a stakeholder needs to try it early. Use the *Epic Generation - Vertical Slice* skill for that.
...
- **Prefer independent epics** — the point of this strategy is parallel execution, so favour seams that let two epics be built at the same time without touching the same files. Where a real prerequisite exists, declare it in `Depends On:` rather than merging the epics.
- **Design the shared interfaces up front** — the foundational epics should specify the contracts (schema, service signatures, component APIs) that later epics consume, so downstream epics can be built against them without rework.
