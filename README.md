...
- `tickets create` runs the *Ticket Generation* skill against an epic and writes the tickets to the backlog database. Tickets are written straight to `todo` by default so the scheduler can pick them up, and `--assignee` is then required: the scheduler never dispatches a `todo` ticket without an agent, so tickets written without one would sit there forever. `--status backlog` writes them the way the workbench does (not dispatchable until promoted). The CLI enforces the status and assignee on every ticket it created, whatever the agent wrote, and flips the epic's `Status:` to `TICKETS`. `--closeout` controls which of the skill's four mandatory closeout tickets are appended: `all` (default, as in the workbench), `eval` (only the epic-level evaluation ticket — for repositories with no help docs or code map to maintain), or `none`.
...
- `finalize` runs once, after the drain, in the real working tree on `main` — not a worktree. Tickets build, test and evaluate in isolated worktrees and reach `main` as squash-merged source only, so anything a ticket did to its *environment* (an editable install that puts a CLI on `PATH`, artefacts written by running the delivered tool) is discarded with the worktree. `finalize` performs the epic's installation steps, executes its acceptance and validation commands from the repository root, fixes what fails, and commits source changes; generated artefacts stay in place. It is the step where a merged feature is confirmed to work from a clean checkout.
...
node .ombutocode/src/headless.js finalize --epic docs/Epics/epic_01_FEATURE.md --input spec/feature.md --json
```
